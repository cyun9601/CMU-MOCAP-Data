...
def draw(skeletons, paris, blank_position=None, figsize=(10, 10), yz_rotation=False,
         elev=None, azim=None, title=None, font_size=10, axis_off = False, save=None, dpi=300) : 
    ...
    for i, skeleton in enumerate(skeleton_data) : 
        ...
        ax = fig.add_subplot('1' + str(N) + str(i+1), projection='3d')
        plt.title(title)
        ...
        if tuple_flag : # skeleton 데이터가 두개 
            ...
            if yz_rotation: # yz축 swap
                y_new = np.array(skeleton1[2, :])
                z_new = np.array(skeleton1[1, :])                                
...
def pkl_draw(directory1, data_index, sequence, paris, unnorm=False, yz_rotation=False, elev=None, azim=None, directory2=None) : 
    '''
    Data, Mean, Max_val로 구성된 pkl 파일에서 특정 Sequence 데이터를 Draw.
    ...
    with open(directory1, 'rb') as f:
        data1, pose_mean, pose_max = pickle.load(f).values()        

    if directory2 != None : 
        with open(directory2, 'rb') as f:
            data2, pose_mean, pose_max = pickle.load(f).values()        
    ...
    if directory2 == None : 
        draw(data1[data_index, :, sequence, :, 0], paris, yz_rotation=yz_rotation, elev=elev, azim=azim) # N, C, T, V, M
    else : 
        draw((data1[data_index, :, sequence, :, 0], data2[data_index, :, sequence, :, 0]), paris, yz_rotation=yz_rotation, elev=elev, azim=azim) # N, C, T, V, M

    
    
    
def animation(skeleton, paris, missing_position=None, figsize=(10,10), elev = None, azim = None, title = None, font_size=10, axis_off = False, axis_equal = True) : 
